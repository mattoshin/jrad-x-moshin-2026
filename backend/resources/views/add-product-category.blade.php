...
<?php
	$mode = "";
	if(isset($category_data->type) ? $category_data->type : '' ){
		$mode ="add";
	}
	else{
		$mode ="edit";
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title mb-4">Product Category</h4>
                                	@if($mode =='add')
	                                <form class="outer-repeater" name="edit-category" method="post" action="{{ route('editCategory_save') }}">
                                	@else
	                                <form class="outer-repeater" name="add-category" method="post" action="/admin/save-category">
                            		@endif
                                    @csrf
                                    <input type="hidden" name="category_id" value="{{ isset($category_data->id) ? $category_data->id : ''}}">
                                    <div data-repeater-list="outer-group" class="outer">
                                        <div data-repeater-item="" class="outer">
                                            <div class="form-group mb-4 col-12">
                                                <label for="taskname" class="col-form-label">Category name</label>
                                                <input id="cate_name" name="cate_name" type="text" class="form-control" placeholder="Enter category name" value="{{ isset($category_data->name) ? $category_data->name : '' }} " required>
                                                @error('cate_name')
                                            ...
                                            <div class="form-group mb-4 col-12">
                                                ...
                                                <textarea id="cate_descr" class="form-control" name="cate_descr" required>{{ isset($category_data->description) ? $category_data->description : ''}}</textarea>
                                                @error('cate_descr')
			                                        <span class="text-danger">
			                                            <strong>
                                            ...
                                            <div class="col-3">
                                                <label for="taskbudget" class="col-form-label" required>Category Type</label>
                                                <select name="cate_type" id="cate_type" class="form-select">
                                                	<option value="">-- Select --</option>
                                                    <option {{ (isset($category_data->type) ? $category_data->type : '' == '1') ? 'selected' : '' }} value="1">Curated</option>
                                                    <option {{ (isset($category_data->type) ? $category_data->type : '' == '2') ? 'selected' : '' }} value="2">Webhook</option>
                                                </select>
                                                @error('cate_type')
			                                        <span class="text-danger">
			                                            <strong>
