...
class Monitor extends Component{
    ...
    componentDidMount=e=>{
        ...
        handleSubmit(channelObject, currentWebHook){
            ...
            // }
            if (channelObject){
                if (channelObject.webhook_url.match(/discord.com\/api\/webhooks\/([^\/]+)\/([^\/]+)/)) {
                    let changedObject = {
                ...
                if (currentWebHook.weebHookUrl.match(/discord.com\/api\/webhooks\/([^\/]+)\/([^\/]+)/)) {
                    let changedObject = {
        ...
        handleUrlChange = (e, changedHook, isPlan) => {
           if (!isPlan) {
            ...
            if (!isExisting){
                ...
                this.setState({
                    changes: this.state.changes.map(change => {
                        if (change.channel_id === changedHook.id){
                            return {
                                ...change,
                                weebHookUrl: e.target.value
                            }
        ...
        const names = this.state.webHooksData.map((name, i)=>{
            ...
            return(
                <tr class="">
                    ...
                    <td className="col-1">
                    <button type="button" onClick={() => this.handleSubmit(currentPlan, name)} className="table-button">Save</button>
                    </td>
                </tr>
            )
        })
    return(
        <div id="monitorsPanel">
            <Toaster/>
            <div id="monitorsHeader">
                <h6 id="monitorsHeaderText" class ="">Monitors</h6>
                <button type="button" id="saveButton">Save Webhooks</button>
            </div>
